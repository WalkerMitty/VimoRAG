...
import math
from videogpt_plus.model.internvideo_1.modules.until_config import PretrainedConfig
...
class LossWeight(nn.Module):
    ...
    def forward(self,weight_matrix):
        #输入是方阵，大小是n*n,希望返回所有值的mean()，但是不包括对角线上的值，这里似乎可以省略，但是目前先不做省略
        n = weight_matrix.size(0)
        mask = torch.ones(n, n, device=weight_matrix.device) - torch.eye(n, device=weight_matrix.device)
        masked_matrix = weight_matrix * mask
        return masked_matrix.mean()
