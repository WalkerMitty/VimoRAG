...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer,
        ...
        self.k = data_args.k 
        self.motion_encoder = data_args.motion_encoder
        ...
        if 'image' in sources:
            ...
            if self.k!=1:
                ...
                frames, context_images = _get_rawvideo_dec(video_file_path, image_processor, video_processor,
                                                        ...
                                                        num_context_images=NUM_CONTEXT_IMAGES)
                if self.motion_encoder:
                    keypoints = self._get_motion_motionbert([video_file_path],16)
                else:
                    keypoints = None                                                            
            sources = preprocess_multimodal(
                copy.deepcopy([e["conversations"] for e in [sources]]),
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in instances[0]:  
            ...
            keys = [instance['keypoints'] for instance in instances] 
            ...
            if keys[0] is not None:
                batch['keypoints'] = torch.cat(keys,dim=0) 
            else:
