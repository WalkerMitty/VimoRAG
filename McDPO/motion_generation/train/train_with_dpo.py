...
from videogpt_plus.train.dpo_trainer import VideoGPTPlusDPOTrainer
...
import random
import numpy as np
from videogpt_plus.model.dataloader import _get_rawvideo_dec,build_clip_model,_get_fixed_text_fea
import os
...
from videogpt_plus.model.internvideo_1.modules.tokenization_clip import SimpleTokenizer as ClipTokenizer



from new_trl.trainer.utils import DPODataCollatorWithPadding
...
class ModelArguments:
    ...
    motion_tower: Optional[str] = field(default=None)
    fusion_model: Optional[str] = field(default=None)
    image_vision_tower: Optional[str] = field(default=None)
    mm_vision_select_layer: Optional[int] = field(default=-1)  
    pretrain_mm_mlp_adapter: Optional[str] = field(default=None)
    pretrain_motion_mm_mlp_adapter: Optional[str] = field(default=None)
    ...
    sft_model: Optional[str] = field(default=None)
...
class DataArguments:
    ...
    dataset_use: str = field(default="DPO")
...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ['image_mm_projector', 'mm_projector','motion_mm_projector','vision_tower', 'image_vision_tower','motion_tower']
...
def preprocess_phi3(
    ...
    if k!=1:
        temp_con = conversations.copy()
        conversations = []
        for item in temp_con:
            for _ in range(k):
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version == "phi3":
        return preprocess_phi3(sources, tokenizer, has_image=has_image,k=k) 
...
class LazyDPODataset(Dataset):
    ...
    def _get_motion_motionbert(self,video_paths,max_frames):
        all_keypoints = []
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources:
            ...
            if self.k!=1:
                ...
                for i in range(self.k):
                    all_video_files.append(sources['video'+str(i+1)]) 
            if 'workspace' in video_file:
                video_file = video_file.replace('workspace','data/nas')
                if self.k!=1:
                    temp = all_video_files.copy()
                    all_video_files = []
                    for item in all_video_files:
                        all_video_files.append(item.replace('workspace','data/nas'))
            ...
            if self.k!=1:
                ...
                init_smpl = []
                key_mask = []
                for cur_video in all_video_files:
                    frame, context_image, cur_keypoints, cur_init_kp, cur_init_smpl, cur_key_mask = _get_rawvideo_dec(cur_video, image_processor, video_processor,
                                        frame_resolution=224, max_frames=NUM_FRAMES,
                                        num_video_frames=NUM_FRAMES,
                                        num_context_images=NUM_CONTEXT_IMAGES,
                                        detector=self.detector,extractor=self.extractor,
                                        smpl=self.smpl,keypoints_normalizer=self.keypoints_normalizer)
                    frames+= frame
                    context_images += context_image
                    keypoints.append(cur_keypoints)
                    init_kp.append(cur_init_kp)
                    init_smpl.append(cur_init_smpl)
                    key_mask.append(cur_key_mask)
                keypoints = torch.cat(keypoints,dim=0)
                init_kp = torch.cat(init_kp,dim=0)
                init_smpl = torch.cat(init_smpl,dim=0)
                key_mask = torch.cat(key_mask,dim=0)
        ...
        if isinstance(i, int):
            ...
            data_dict = dict(chosen_input_ids=data_dict["chosen_input_ids"][0], chosen_labels=data_dict["chosen_labels"][0],rejected_input_ids=data_dict["rejected_input_ids"][0],
...
@dataclass
class DataCollatorForDPODataset(DPODataCollatorWithPadding):
    tokenizer: transformers.PreTrainedTokenizer

    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        chosen_input_ids, chosen_labels,rejected_input_ids,rejected_labels = tuple([instance[key] for instance in instances]
...
def make_dpo_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    train_dataset = LazyDPODataset(tokenizer=tokenizer, data_args=data_args)
...
def train():
    ...
    model.config.k = data_args.k
    if data_args.text_only:
    ...
    if model_args.tune_mm_mlp_adapter:
        ...
        for p in model.get_model().image_mm_projector.parameters():
            p.requires_grad = True
        for p in model.get_model().motion_mm_projector.parameters():
            p.requires_grad = True
    ...
    if data_args.train_stage in [0,1]:
        trainer = VideoGPTPlusTrainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
    elif data_args.train_stage in [2]:
        
        model.load_adapter("/path/to/peft", adapter_name="reference") 
        
        
        trainer = VideoGPTPlusDPOTrainer(model=model, processing_class=tokenizer, args=training_args_dpo, **data_module_dpo)
    else:
        raise ValueError()
...
from videogpt_plus.model.builder import load_pretrained_model,merge_peft_model
...
def train_with_dpo():
    ...
    model_path = model_args.sft_model
    ...
    data_args.mm_use_im_start_end = mm_use_im_start_end
    model.config.mm_projector_lr = training_args.mm_projector_lr
    ...
    trainer = VideoGPTPlusDPOTrainer(
        model,
        **data_module,
        args=training_args,
