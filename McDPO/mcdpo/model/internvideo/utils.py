...
from mcdpo.model.internvideo.internvideo2 import pretrain_internvideo2_1b_patch14_224
from mcdpo.model.internvideo.pos_embed import interpolate_pos_embed_internvideo2_new
...
class InternVideo2_Stage2V(nn.Module):
    ...
    def __init__(self, config, is_pretrain: bool = True):
        ...
        self.image_processor = VideoTrainProcessor(num_frames=self.vision_encoder.num_frames) #num_frames 4
    ...
    def forward(self, image):
        T = image.shape[1]
        use_image = True if T == 1 else False
        image = image.permute(0, 2, 1, 3, 4).to(self.dtype)  # [B,T,C,H,W] -> [B,C,T,H,W] torch.Size([4, 3, 4, 224, 224]) bsz, channel, frames, h, w
        # import ipdb;ipdb.set_trace()
