...
from dataloaders.dataloader_motionx_retrieval import MOTIONX_DataLoader, MOTIONX_TrainDataLoader

def dataloader_motionx_train(args, tokenizer):
    motionx_dataset = MOTIONX_TrainDataLoader(
    ...
    return dataloader, len(motionx_dataset), train_sampler

def dataloader_motionx_test(args, tokenizer, subset="test"):
    num = args.num if hasattr(args, 'num') else -1
    threads = args.threads if hasattr(args, 'threads') else 8
    bsz = args.batch_size_val if hasattr(args, 'batch_size_val') else 32
    motionx_testset = MOTIONX_DataLoader(
    ...
    return dataloader_motionx, len(motionx_testset)
...
DATALOADER_DICT["motionx"] = {"train":dataloader_motionx_train, "val":dataloader_motionx_test, "test":None}
