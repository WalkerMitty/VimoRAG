...
def load(
    ...
    # import InternVideo
    # model = InternVideo.load_model("./models/InternVideo-MM-L-14.ckpt")
    state_dict = {}
    for k, v in init_state_dict.items():
        k = k.replace('clip.','')
        state_dict[k] = v

    if not jit:  #下面传入clip_state_dict是原始vit-l/14的参数，实际上没有用到，因为mergeweight是false，model用intervideo初始化了
        model = build_model(
