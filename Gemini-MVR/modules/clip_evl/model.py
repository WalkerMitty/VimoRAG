...
class CLIP(nn.Module):
    def __init__(self,
        ...
        # if self.sim_header !='weightPrior' or self.stage ==2:
    ...
    def encode_video(self, video, return_all_feats=False, masked_indices=None, mode="video"):
        # video: [N, C, T, H, W] TODO:self.visual  modules/clip_evl/evl_utils/clip_vit_only_global.py code似乎和生成阶段不一致，最后check
  
        feats = self.visual(video, return_all_feats=return_all_feats, mode=mode)
        if return_all_feats:
            x, feats = feats
        #feats: 257,bsz, 12, 1024]
        x = self.visual_ln_post(x)
        
        #our add <<<
        feats = self.visual_ln_post(feats)[0,:,:,:]
        # feats = self.visual_ln_post(feats)
        # feats = torch.mean(feats, dim=0).squeeze(0)
        # >>>
        if self.visual_proj is not None:
            x = x @ self.visual_proj

            #<< our add
            # feats = feats @ self.visual_proj
            feats = feats @ self.visual_proj
            #>>>. [bsz, 12, 768]
    ...
    def encode_text(self, text, masked_indices=None, return_all_feats=False):
        ...
        #[bsz, 77, 768]
        x = x.permute(1, 0, 2)  # NLD -> LND. #这里必须是[77,bsz, 768]
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x).type(self.dtype)

        # x.shape = [batch_size, n_ctx, transformer.width]    feats:  [bsz, 768]
